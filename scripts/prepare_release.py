...
# dependencies = ["typer>=0.26.1"]
# ///

"""Prepare a release by updating the plugin version and release notes."""
...
def get_current_version(content: str, version_file: Path) -> str:
    ...
    if not isinstance(data, dict):
        raise RuntimeError(f"Expected a JSON object in {version_file}")
    version = data.get("version")
    if not isinstance(version, str):
        raise RuntimeError(f"Expected a string version in {version_file}")
...
def parse_manifest_files(value: str) -> list[Path]:
    paths = [Path(item.strip()) for item in value.split(",") if item.strip()]
    if not paths:
        raise RuntimeError("At least one plugin manifest file is required")
    if len(paths) != len(set(paths)):
        raise RuntimeError("Plugin manifest files must be unique")
    return paths
...
def prepare(
    ...
    version_file: Annotated[
        ...
        typer.Option(
            ...
            readable=True,
            writable=True,
            help="Path to the JSON file containing the canonical plugin version.",
        ),
    ],
    plugin_manifest_files: Annotated[
        str,
        typer.Option(
            envvar="PREPARE_RELEASE_PLUGIN_MANIFEST_FILES",
            help="Comma-separated paths to directly installable plugin manifests.",
        ),
    ],
    ...
    manifest_files = parse_manifest_files(plugin_manifest_files)
    for manifest_file in manifest_files:
        if not manifest_file.is_file():
            raise RuntimeError(f"Plugin manifest does not exist: {manifest_file}")

    version_file_content = version_file.read_text()
    manifest_file_contents = {
        manifest_file: manifest_file.read_text() for manifest_file in manifest_files
    }
    release_notes_content = release_notes_file.read_text()

    current_version = get_current_version(version_file_content, version_file)
    for manifest_file, manifest_content in manifest_file_contents.items():
        manifest_version = get_current_version(manifest_content, manifest_file)
        if current_version != manifest_version:
            raise RuntimeError(
                f"Version mismatch: {version_file} has {current_version}, "
                f"but {manifest_file} has {manifest_version}"
            )
    version = bump_version(current_version, bump)

    updated_version_file_content = update_version_file(
        version_file_content, version, version_file
    )
    updated_manifest_file_contents = {
        manifest_file: update_version_file(
            manifest_content, version, manifest_file
        )
        for manifest_file, manifest_content in manifest_file_contents.items()
    }
    updated_release_notes_content = update_release_notes(
        release_notes_content, version, parsed_release_date, release_notes_file
    )

    version_file.write_text(updated_version_file_content)
    for manifest_file, manifest_content in updated_manifest_file_contents.items():
        manifest_file.write_text(manifest_content)
    release_notes_file.write_text(updated_release_notes_content)
